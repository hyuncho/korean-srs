...
- **Drill to 95% — two formats** (pick on the home screen; default = Continuous):
  - **Continuous:** goes through all ~20 words once, then keeps serving words drawn from the whole list but **weighted toward ones you've missed** (never the same word twice in a row). Ends when your **cumulative** pass rate reaches **95%** (only checked after that first full pass).
  - **Rounds:** repeated **full passes** through the list; each round shows every word once and ends when one *whole pass* scores **≥ 95%**.
  - Either way the screen shows correct count, wrong count, and the live pass rate (no progress bar).
